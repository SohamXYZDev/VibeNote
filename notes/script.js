...
function closeFullNote(caller) {
    ...
    editNote(noteId, updatedNote.title, updatedNote.content, fullNoteEmotionTags.innerHTML , is_pinned); 
...
function editNote(caller, title, content, emotion_tags_passed, pinned = false){
    ...
    const emotion_tags = note.querySelector(".emotion_tags"); 
...
function addNote() {
    localStorage.setItem("noteNo", parseInt(localStorage.getItem("noteNo")) + 1 || 1); 
    ...
    newNote.id = "note" + localStorage.getItem("noteNo"); 
...
function togglePinnedNotesHeading() {
    ...
    const pinnedHeading = document.getElementById("pinned_notes_heading")
...
function getTagColor(tag) {
    ...
    return tagColors[tag.toLowerCase()] || "#e2e3e5"; 
}
...
function addNoteContent(newNote, title, content, tags, pinned = false) {
    ...
    deleteButton.onclick = function (event) {
        localStorage.setItem("noteNo", parseInt(localStorage.getItem("noteNo")) - 1);
        event.stopPropagation(); 
        localStorage.removeItem(newNote.id);
        newNote.remove(); 
        reorderNotes();
        togglePinnedNotesHeading(); 
    };
...
function reorderNotes() {
    ...
    const notes = noteContainer.querySelectorAll(".note-card"); 
    let newNoteNo = 0;
